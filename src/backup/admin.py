...
from backup.models import ArquivoBackup, DonoArquivoBackup, DonoBackup
...
class DonoArquivoAdmin(BaseModelAdmin):
    class DonoArquivoBackupResource(ModelResource):
        arquivo_backup = Field(
            "arquivo_backup",
            "arquivo_backup",
            ForeignKeyWidget(ArquivoBackup, field=ArquivoBackupAdmin.Resource.Meta.import_id_fields[0]),
        )
        dono_backup = Field(
            "dono_backup",
            "dono_backup",
            ForeignKeyWidget(DonoBackup, field=DonoBackupAdmin.Resource.Meta.import_id_fields[0]),
        )
