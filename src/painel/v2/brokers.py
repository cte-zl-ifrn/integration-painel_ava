...
class SuapBroker:
    ...
    def generate_pair(self, username: str, access_days: int = 1, refresh_days: int = 7) -> dict:
        now = datetime.datetime.now(timezone.utc).timestamp()
        jwt_secret = getattr(settings, "JWT_SECRET", "secret")
        access_exp = now + access_days * 24 * 60 * 60
        refresh_exp = now + refresh_days * 24 * 60 * 60

        access_token = jwt.encode(
            {"username": username, "exp": access_exp, "type": "access"},
            jwt_secret,
            algorithm="HS256",
        )
        refresh_token = jwt.encode(
            {"username": username, "exp": refresh_exp, "type": "refresh"},
            jwt_secret,
            algorithm="HS256",
        )
